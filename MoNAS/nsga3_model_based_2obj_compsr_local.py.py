...
parser.add_argument("--seed", type=int, default=1, help="Seed for NSGA-III run")
parser.add_argument("--model-path", type=str, default="Model_Based/final_model_knn.pkl", help="Ruta del modelo KNN")
    ...
    for i in range(len(gens)):
        ...
        for layer in gens[i]:
            ...
            if layer[0] == 'conv':
                ...
                for l in range(layer[2]):
                    branches[i].extend(
                        [layers.DepthwiseConv2D(layer[1], **conv_args), layers.Conv2D(channels[i][1], 1, **conv_args)])

            elif layer[0] == 'invert_Bot_Conv_E2':
                expand = float(channels[i][1]) * 2
                for l in range(layer[2]):
                    branches[i].extend(
                        [layers.Conv2D(expand, 1, **conv_args), layers.DepthwiseConv2D(layer[1], **conv_args),
                         layers.Conv2D(channels[i][1], layer[1], **conv_args)])
...
class NSGA3:
    ...
    def _normalize(self, s, pop):
        ...
        for index in valid_s:
            ind = [
                (pop[index][obj] - min_obj[obj]) / (max_obj[obj] - min_obj[obj]) if max_obj[obj] != min_obj[obj] else 0
                for obj in range(self.problem.n_obj)]
            ind.append(index)
...
print(problem)
nsga3 = NSGA3(pop_size=20, n_gen=1000, problem=problem, verbose=True)
