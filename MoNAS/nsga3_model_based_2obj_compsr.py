...
parser = argparse.ArgumentParser(description="Ejecutar una seed de NSGA-III (surrogate)")
parser.add_argument("--seed", type=int, required=True,
                    help="Seed para este experimento")
parser.add_argument("--model-path", type=str,
                    default="models/final_model_hgbr.pkl",
                    help="Ruta al modelo predictor")
...
seed_folder = os.path.join(args.outdir, f"seed_{seed_value}")
os.makedirs(seed_folder, exist_ok=True)

print(f"[INFO] Running NSGA-III with seed={seed_value}")
...
print(f"Running NSGA-III with seed {seed_value}, model {model_path}")
...
class NSGA3:
    ...
    def _do(self):
        ...
        while c < self.n_gen:
            ...
            model_names = model_path.split('/')[-1].replace('.pkl', '').split('_')[-1]

            filename = f"{model_names}_Output_Population_seed_{seed_value}.txt"
            with open (filename,"a") as file:
                for x,f in zip(pop["X"],pop["F"]):
                    file.write(f"{x}" + f"{f}" + "\n" )
            c += 1
...
class ModifiedMyPMOP:
    def __init__(self, model_path=model_path, n_var=84, n_obj=2): #Just change the model path
        self.n_var = n_var
        self.n_obj = n_obj
...
print(problem)
nsga3 = NSGA3(pop_size=100, n_gen=500, problem=problem, verbose= True)
